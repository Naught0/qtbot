...
class Google(commands.Cog):
    ...
    BING_H = {
        "User-Agent": "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/4.0; GTB7.4; "
    ...
    async def _google(self, ctx, *, query):
        ...
        # Actually request page html and store the link list for 6 hrs
            ...
            resp_html = await aw.aio_get_text(
                self.aio_session,
                self.scrape_uri,
                params={"q": query},
    ...
    def _make_image_embed(self, query: str, html: str) -> List[discord.Embed]:
        ...
        link_list = [
            x.get("href")
            for x in root.xpath('//div[@class="content"]//a[@class="thumb"]')
        ]
    ...
    async def bing_image_search(self, ctx, *, query):
        ...
        params = {"q": query}
        html = await aw.aio_get_text(
            self.aio_session, self.BING_URI, params=params, headers=self.BING_H
        )
