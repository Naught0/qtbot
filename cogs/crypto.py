...
class Crypto(commands.Cog):
    ...
    async def bitcoin(self, ctx):
        ...
        em.add_field(
            name="Price USD", value=f"${float(resp['price_usd']):,.2f}", inline=False
        )
        ...
        change_1h_str = (
            f":arrow_up: {change_1h}%"
            if "-" not in change_1h
            else f":arrow_down: {change_1h}%"
        )
        ...
        change_24h_str = (
            f":arrow_up: {change_24h}%"
            if "-" not in change_24h
            else f":arrow_down: {change_24h}%"
        )
        ...
        change_7d_str = (
            f":arrow_up: {change_7d}%"
            if "-" not in change_7d
            else f":arrow_down: {change_7d}%"
        )
