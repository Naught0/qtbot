...
class Giphy:
    ...
    async def giphy(self, ctx, *args):
        ...
        if gif_result:
            ctx.send(gif_result.media_url)
        else:
            ctx.send("Sorry, I couldn't find anything for `{}`.".format(q))
    ...
    async def thanks(self, ctx):
        ...
        await ctx.send(gif.fixed_width.downsampled.url)
