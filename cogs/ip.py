...
class IPLookup:
    ...
    async def iplookup(self, ctx, *, query: str):
        ...
        em.set_image(
            url=f"https://maps.googleapis.com/maps/api/staticmap?center={res['lat']},{res['lon']}&zoom=13&scale=false&size=255x255&maptype=roadmap&format=png&markers=size:mid%7Ccolor:0xAD1457%7Clabel:%7C{res['lat']},{res['lon']}#{str(datetime.datetime.now())}")
