...
class DownDetect:
    ...
    @commands.bot.command(name="isup", aliases=["dd"])
    async def down_detector(self, check_url):
        ...
        if "not" in isup_result:
            return await self.bot.say("`{}` Looks down from here.".format(check_url))
        else:
            return await self.bot.say("`{}` Looks up from here.".format(check_url))
