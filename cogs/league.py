...
class League(commands.Cog):
    ...
    @staticmethod
    def _make_champ_embed(
        champ_dict: dict, riot_champ_name: str, fancy_champ_name: str, champ_title: str
    ) -> discord.Embed:
    ...
    async def get_league_elo(self, ctx, *, summoner=""):
        ...
        # Store the data for 2hrs because it doesn't update that frequently
            ...
            elo_data = await aw.aio_get_json(
                self.session,
                self.elo_api_uri.format(f_summoner),
                headers=self.elo_headers,
            )
        ...
        if elo_data["ARAM"]["avg"]:
            em.add_field(
                name="ARAM MMR",
                value=f"{elo_data['ARAM']['avg']}±{elo_data['ARAM']['err']}",
            )
